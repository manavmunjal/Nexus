...
public final class Main {
     ...
     */
    private static final String DEFAULT_DATASET = "src/main/resources"
    ...
    public static void runAnalysis(final Config cfg) throws Exception {
        ...
        final ScoreMapper scoreMapper = ScoreMapper
                .fromAttribute(classAttribute);

        final DataSplitter.Split split = DataSplitter
                .split(data, cfg.getTrainRatio(), cfg.getSeed());

        final SentimentModelTrainer trainer = new SentimentModelTrainer();
        final FilteredClassifier classifier = trainer
                .train(split.train(), cfg.getTextAttribute());
        ...
        limit = Math.min(limit, predictions.size()); // don't exceed pred. count
        ...
        final Map<String, GroupStatistics> productSummaries = ReportPrinter
                .printGroupSummaries(
                        "Product Summaries",
                        byProduct,
                        classValues);

        final Map<String, GroupStatistics> companySummaries = ReportPrinter
                .printGroupSummaries(
                        "Company Summaries",
                        byCompany,
                        classValues);
